...
// 全局变量：记录当前无人艇状态、目标偏航角、误差和 PID 参数
...
// PID 控制器函数
float calculateAngularVelocity(float error) {
    // 计算时间间隔
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "boat_run_with_yaw_pid_control");
    ros::NodeHandle nh;
    ...
    cmd_vel_msg.linear.y = 0.5;  // 固定前进速度
    ...
    while (ros::ok()) {
        ...
        // 计算角度误差（目标角度减当前角度，假设当前角度为 0）
        float angle_error = target_yaw_angle;
        ...
        if ((ros::Time::now() - last_detection_time).toSec() > 10.0) {
            ...
            // 使用 PID 控制器计算角速度
            cmd_vel_msg.angular.z = calculateAngularVelocity(angle_error);
        }
