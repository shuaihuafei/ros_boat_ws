...
int main(int argc, char** argv) {
    ...
    while (ros::ok()) {
        ...
        if ((ros::Time::now() - last_detection_time).toSec() > 10.0) {
            ...
            cmd_vel_msg.linear.y = 0.0;  // 清除角度大小
            ROS_WARN("No target detected for 10 seconds. Stopping rotation.");
        } else {
            // 根据目标角度调整转向方向和角度大小
            //cmd_vel_msg.linear.x = (target_yaw_angle >= 0) ? 1.0 : -1.0;  // 设置方向
            cmd_vel_msg.linear.x = target_yaw_angle;                 // 设置角度大小的绝对值
            cmd_vel_msg.linear.y = fabs(target_yaw_angle);                 // 设置角度大小的绝对值
        }
