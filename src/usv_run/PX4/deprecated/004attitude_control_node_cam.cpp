...
public:
    ...
    void sendAttitudeControl() {
        ...
        double roll = 0.0;
        double pitch = 0.0;
        double yaw = cam_deg_kp * cam_yaw_angle * M_PI / 180.0 + current_yaw;  // 当前yaw值加上pose中的yaw
    ...
    void actuatorControlCallback(const mavros_msgs::ActuatorControl::ConstPtr& msg) {
        ...
        if (msg->controls.size() >= 4) {
            ...
            ROS_INFO_THROTTLE(1, "Actuator 2 control signal in degrees: %f", control_signal_2_in_degrees);
        } else {
    ...
    void objectAngleCallback(const std_msgs::Float64::ConstPtr& msg) {
        cam_yaw_angle = -1 * msg->data;  // 获取目标偏航角
