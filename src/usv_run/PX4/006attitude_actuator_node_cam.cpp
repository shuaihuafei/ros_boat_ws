...
public:
    explicit AttitudeControlNode(ros::NodeHandle& nh) : nh_(nh) {
        ...
        ROS_INFO("kp_d: %f, deg_th: %f, pid_throttle_output: %f", kp_d, deg_th, pid_throttle_output);
        ...
        cam_yaw_sub = nh.subscribe("/yolo/dock_angle", 10, &AttitudeControlNode::objectAngleCallback, this);
        ...
        if (output_file_pid.is_open()) {
            output_file_pid << "time,position_x,position_y,position_z,yaw_angle,v_body_x,cam_yaw_angle,current_yaw,desired_yaw,des_cur_error,kp_d,deg_th,pid_yaw_output,pid_throttle_output,control_signal_2,control_signal_3\n";
        } else {
        ...
        if (output_file_pid_test.is_open()) {
            output_file_pid_test << "time,position_x,position_y,position_z,yaw_angle,v_body_x,cam_yaw_angle,current_yaw,desired_yaw,des_cur_error,kp_d,deg_th,pid_yaw_output,pid_throttle_output,control_signal_2,control_signal_3\n";
        } else {
            ROS_ERROR("Failed to open CSV file for logging.");
        }
    }
    ...
    void updateActuatorControl() {
        if(cam_yaw_angle != -1)
            desired_yaw = -1 * cam_yaw_angle * M_PI / 180.0 + current_yaw_record;
    }
    ...
    void sendActuatorControl() {
        if(cam_yaw_angle != -1)
        {
            ...
            // pid_throttle_output = kp_d * (deg_th - fabs(cam_yaw_angle)) * pid_throttle_output + pid_throttle_output;
    ...
    void checkTimeout(const ros::TimerEvent&) {
        ...
        if ((ros::Time::now() - last_received_time).toSec() > 5.0) {
            cam_yaw_angle = -1;
            ROS_WARN("No object angle received for 1 second, setting cam_yaw_angle to -1.");
        }
    }
    ...
    void objectAngleCallback(const std_msgs::Float64::ConstPtr& msg) {
        ...
        ROS_INFO_THROTTLE(1, "Received target yaw angle: %f deg", cam_yaw_angle);
    }
...
private:
    ros::NodeHandle nh_;
    ros::Subscriber state_sub, actuator_control_sub, pose_sub, local_velocity_body_sub, cam_yaw_sub;
