...
angle_pub = rospy.Publisher("/yolo/dock_angle", Float64, queue_size=10)
image_pub = rospy.Publisher("/yolo/predict_dock", Image, queue_size=10)
...
def image_callback(ros_image):
    ...
    try:
        ...
        # Run YOLO prediction on the image, classes=[2]表示第一个分类也就是dock1, verbose=False表示禁用打印信息
        results = model.predict(source=cv_image, show=False, classes=[2], verbose=False)
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # Only process if the class name is "dock"
                # if class_name == "dock":
...
def main():
    ...
    rospy.init_node("dock_image_predictor", anonymous=True)
